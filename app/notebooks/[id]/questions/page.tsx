...
export default function QuestionsPage() {
  ...
  async function fetchData() {
    const [qRes, nbRes] = await Promise.all([
      supabase.from('questions').select('*').eq('notebook_id', id).order('created_at'),
      supabase.from('notebooks').select('name').eq('id', id).single(),
    ])
    if (qRes.data) setQuestions(shuffle(qRes.data))
    if (nbRes.data) setNotebookName(nbRes.data.name)
    setLoading(false)
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
        <div className="text-center max-w-sm">
          ...
          <p className="text-slate-400 mb-6">Importe questões para este caderno primeiro.</p>
          <Link href={`/notebooks/${id}/questions/import`} className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-colors">
            Importar Questões
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-sm w-full text-center">
          ...
          <p className="text-slate-400 mb-6">{notebookName}</p>
          ...
          <div className="flex gap-3">
            <Link href={`/notebooks/${id}`} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-200 py-2.5 rounded-xl text-sm font-medium transition-colors">
              Voltar
            </Link>
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-2 mb-5">
          <span className="text-xs bg-emerald-500/15 text-emerald-400 px-3 py-1 rounded-full font-medium">
            ✓ {score.correct} corretas
          </span>
          {score.total > score.correct && (
            <span className="text-xs bg-red-500/15 text-red-400 px-3 py-1 rounded-full font-medium">
              ✗ {score.total - score.correct} erradas
            </span>
          )}
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {answered && opt.explanation && (isCorrect || isSelected) && (
                      <div className={`mt-2 text-xs leading-relaxed ${isCorrect ? 'text-emerald-400' : 'text-red-400'}`}>
                        {opt.explanation}
                      </div>
                    )}
                  </div>
        ...
        {answered && (
          <button
            onClick={next}
            className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white font-medium py-3.5 rounded-xl transition-colors"
          >
