...
export default function FlashcardsPage() {
  ...
  async function fetchData() {
    const [fcRes, nbRes] = await Promise.all([
      supabase.from('flashcards').select('*').eq('notebook_id', id).order('created_at'),
      supabase.from('notebooks').select('name, color').eq('id', id).single(),
    ])
    if (fcRes.data) setCards(shuffle(fcRes.data))
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
        <div className="text-center max-w-sm">
          ...
          <p className="text-slate-400 mb-6">Importe flashcards para este caderno primeiro.</p>
          <Link href={`/notebooks/${id}/flashcards/import`} className="bg-purple-600 hover:bg-purple-500 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-colors">
            Importar Flashcards
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-sm w-full text-center">
          ...
          <p className="text-slate-400 mb-2">{notebookName}</p>
          <p className="text-slate-500 text-sm mb-6">{cards.length} flashcard{cards.length !== 1 ? 's' : ''} revisado{cards.length !== 1 ? 's' : ''}</p>
          <div className="flex gap-3">
            <Link href={`/notebooks/${id}`} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-200 py-2.5 rounded-xl text-sm font-medium transition-colors text-center">
              Voltar
            </Link>
      ...
      <main className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto w-full px-4 py-8">
        ...
        <div className="flex items-center gap-4 mt-8">
          <button
            onClick={prev}
            disabled={current === 0}
            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed text-slate-300 px-5 py-2.5 rounded-xl text-sm font-medium transition-colors"
          >
            <ChevronLeft size={16} />
            Anterior
          </button>
          <button
            onClick={() => setFlipped((f) => !f)}
            className="bg-slate-800 hover:bg-slate-700 text-slate-300 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors"
          >
            Virar
          </button>
          <button
          ...
          >
            {current + 1 >= cards.length ? 'Concluir' : 'Próximo'}
