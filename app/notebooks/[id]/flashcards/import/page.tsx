...
function ImportFlashcardsContent() {
  ...
  async function fetchTopics() { const { data } = await supabase.from('topics').select('id, name').eq('notebook_id', id).order('created_at'); setTopics(data ?? []) }
  async function fetchSubtopics(topicId: string) { const { data } = await supabase.from('subtopics').select('id, name').eq('topic_id', topicId).order('created_at'); setSubtopics(data ?? []) }
  ...
  async function handleImport() {
    ...
    try {
      ...
      const isHeader = rows[0]?.[0]?.toLowerCase().includes('pergunta') || rows[0]?.[0]?.toLowerCase().includes('frente')
      ...
      if (toInsert.length > 0) {
        const { error: e } = await supabase.from('flashcards').insert(toInsert)
    ...
    <div className="min-h-screen bg-black">
      <header className="border-b border-white/8 bg-black/80 backdrop-blur-xl sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-white">Importar Flashcards</h1>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-4">
          ...
          <div>
            ...
            {!showNewTopic ? (
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowNewTopic(false)} className="bg-white/5 text-white/40 px-3 py-2.5 rounded-xl text-sm">✕</button>
              </div>
            )}
          </div>
          {selectedTopicId && (
            <div>
              ...
              {!showNewSubtopic ? (
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setShowNewSubtopic(false)} className="bg-white/5 text-white/40 px-3 py-2.5 rounded-xl text-sm">✕</button>
                </div>
              )}
            </div>
          )}
        ...
        <div className="bg-white/[0.03] border border-white/8 rounded-xl p-4">
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-white/25 font-mono">pergunta | resposta</p>
              <p className="text-xs text-white/20 mt-1">Cada linha = 1 flashcard · Cabeçalho ignorado automaticamente</p>
            </div>
          </div>
        </div>
        ...
        {/* File upload */}
        <div>
          <label className="block text-xs font-medium text-white/40 mb-2 uppercase tracking-wider">Arquivo</label>
          {file ? (
              ...
              <tbody>
                {preview.map((row, i) => (
                  <tr key={i} className={i === 0 ? 'bg-white/5' : ''}>
                    <td className="px-4 py-2.5 border-b border-white/5 text-white/50 w-1/2 truncate max-w-0">{row[0] || '—'}</td>
                    <td className="px-4 py-2.5 border-b border-white/5 text-white/30 w-1/2 truncate max-w-0">{row[1] || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {result && <div className="flex items-center gap-2 bg-white/5 border border-white/15 text-white/80 rounded-xl px-4 py-3 text-sm"><CheckCircle size={15} />{result.success} flashcard(s) importado(s){result.errors > 0 ? ` · ${result.errors} ignorado(s)` : ''}</div>}
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleImport} disabled={!hasData || importing || !selectedSubtopicId}
            ...
            {importing ? <><div className="w-4 h-4 border-2 border-black/20 border-t-black rounded-full animate-spin" /> Importando...</> : <><Upload size={15} /> Importar Flashcards</>}
          </button>
        </div>
      </main>
    </div>
...
export default function ImportFlashcardsPage() {
  return <Suspense fallback={<div className="min-h-screen bg-black flex items-center justify-center"><div className="w-7 h-7 border-2 border-white/20 border-t-white rounded-full animate-spin" /></div>}><ImportFlashcardsContent /></Suspense>
}
